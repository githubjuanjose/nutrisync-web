...
/* NutriSync Web V2 · idioma (UST-07 G5 · UST-12, 7-sep): EN inline (el del Figma) + data-es inyectado por build.py;
   CA · GL · EU llegan de /assets/i18n/<idioma>.json SOLO cuando ese idioma se elige o es el del sistema (D1 = B).
   Cambia innerHTML desde atributos o del JSON, JAMÁS display (lección r16-F17). Sin JS o sin red se ve el inglés.
   D2 (Juanjo): la web arranca SIEMPRE en el idioma del sistema; ?lang= manda en el enlace que lo lleva; la elección
   manual de la píldora dura la sesión (sessionStorage), no se guarda para siempre. La píldora rota EN→ES→CA→GL→EU
   y enseña el código del idioma ACTUAL; el nombre propio va en title/aria (D5). */
(function () {
  'use strict';
  var KEY = 'ns_lang', LANGS = ['en', 'es', 'ca', 'gl', 'eu'], INLINE = { en: 1, es: 1 };
  var NOMBRE = { en: 'English', es: 'Español', ca: 'Català', gl: 'Galego', eu: 'Euskara' };
  var BUILD = (document.querySelector('meta[name="ns-web2-build"]') || {}).content || '0';
  var cache = {};                                   // idioma → JSON (una petición por idioma y visita)
  function detect() {
    try { var q = new URLSearchParams(location.search).get('lang'); if (q && LANGS.indexOf(q) >= 0) return q; } catch (e) {}
    try { var s = sessionStorage.getItem(KEY); if (s && LANGS.indexOf(s) >= 0) return s; } catch (e) {}
    var nav = String(navigator.language || 'en').toLowerCase().slice(0, 2);
    return LANGS.indexOf(nav) >= 0 ? nav : 'en';
  }
  function pinta(lang, dict) {
    document.documentElement.lang = lang;
    document.querySelectorAll('[data-i]').forEach(function (el) {
      if (!el.hasAttribute('data-en')) el.setAttribute('data-en', el.innerHTML);
      var v = lang === 'en' ? el.getAttribute('data-en') : dict ? dict[el.getAttribute('data-i')] : el.getAttribute('data-' + lang);
      el.innerHTML = (v != null && v !== '') ? v : el.getAttribute('data-en');   // sin texto en ese idioma → inglés, nunca vacío
    });
    document.querySelectorAll('[data-i-ph]').forEach(function (el) {
      ...
      var v = lang === 'en' ? el.getAttribute('data-en-ph') : dict ? dict[el.getAttribute('data-i-ph')] : el.getAttribute('data-' + lang + '-ph');
      el.setAttribute('placeholder', (v != null && v !== '') ? v : el.getAttribute('data-en-ph'));
    });
    document.querySelectorAll('[data-lang-switch]').forEach(function (b) {
      b.textContent = lang.toUpperCase();
      var sig = LANGS[(LANGS.indexOf(lang) + 1) % LANGS.length];
      b.setAttribute('title', NOMBRE[lang] + ' · ' + LANGS.map(function (l) { return l.toUpperCase(); }).join(' · '));
      b.setAttribute('aria-label', NOMBRE[lang] + '. ' + NOMBRE[sig]);
    });
    document.dispatchEvent(new CustomEvent('ns-lang', { detail: lang }));
  }
  function carga(lang) {
    if (cache[lang]) return Promise.resolve(cache[lang]);
    return fetch('/assets/i18n/' + lang + '.json?v=' + encodeURIComponent(BUILD), { credentials: 'omit' })
      .then(function (r) { if (!r.ok) throw new Error(String(r.status)); return r.json(); })
      .then(function (j) { cache[lang] = j; return j; });
  }
  // Burbuja (Juanjo, 7-sep 22:1x): lo que enseña el title al pasar el ratón, pero solo al ELEGIR idioma y sin ratón —
  // aparece bajo la píldora pulsada 1,8 s con el nombre propio del idioma actual y la lista.
  var tip = null, tipTimer = 0;
  function burbuja(btn, lang) {
    if (!tip) { tip = document.createElement('div'); tip.className = 'ns-lang-tip'; tip.setAttribute('role', 'status'); tip.setAttribute('aria-live', 'polite'); document.body.appendChild(tip); }
    tip.textContent = NOMBRE[lang] + ' · ' + LANGS.map(function (l) { return l.toUpperCase(); }).join(' · ');
    var r = btn.getBoundingClientRect(); tip.classList.add('on');
    var w = tip.offsetWidth, x = Math.min(Math.max(8, r.left + r.width / 2 - w / 2), innerWidth - w - 8);
    tip.style.left = x + 'px'; tip.style.top = (r.bottom + 8) + 'px';
    clearTimeout(tipTimer); tipTimer = setTimeout(function () { tip.classList.remove('on'); }, 1800);
  }
  var cur = 'en', turno = 0;
  function apply(lang) {
    var mio = ++turno; cur = lang;
    try { sessionStorage.setItem(KEY, lang); } catch (e) {}
    if (INLINE[lang]) { pinta(lang, null); return Promise.resolve(lang); }
    return carga(lang).then(function (dict) { if (mio === turno) pinta(lang, dict); return lang; })
      .catch(function () { if (mio === turno) pinta('en', null); return 'en'; });   // sin red o sin fichero: inglés (r12-b9)
  }
  window.nsLangReady = apply(detect());
  document.querySelectorAll('[data-lang-switch]').forEach(function (b) {
    b.addEventListener('click', function () { var sig = LANGS[(LANGS.indexOf(cur) + 1) % LANGS.length]; window.nsLangReady = apply(sig).then(function (l) { burbuja(b, l); return l; }); });
  });
